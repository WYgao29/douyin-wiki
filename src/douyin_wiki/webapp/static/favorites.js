(() => {
  ...
  async function request(path, body) {
    ...
    if (!response.ok) throw new Error(typeof data.detail === "string" ? data.detail : "请求未完成，请检查选项后重试");
    return data;
  }
  function stopPolling() { clearTimeout(timer); timer = null; }
  function controls() {
    dialog.querySelectorAll("button[data-action]").forEach(button => { button.disabled = busy; });
    ["scope", "history", "filter", "query", "images"].forEach(id => { $(id).disabled = busy; });
    $("scan").disabled = busy || ($("scope").value === "folders" && !$("folders").querySelector("input:checked"));
    const canSelect = current?.status === "needs_selection" && !current.directory_only && !current.confirmed;
    $("confirm").disabled = busy || !canSelect || !current.summary.selected || (!current.complete && !$("partial").checked);
    ["select-all", "exclude-all"].forEach(id => { $(id).disabled = busy || !canSelect; });
    $("previous").disabled = busy || !current || page <= 1;
    $("next").disabled = busy || !current?.has_more;
    $("retry").disabled = busy || !current || !(terminal.has(current.status) || current.summary.failed);
    dialog.querySelectorAll("input[data-work]").forEach(input => { input.disabled = busy || !canSelect || input.dataset.eligible !== "true"; });
  }
  async function run(action) {
    if (busy) return;
    busy = true; stopPolling(); generation++; $("error").textContent = ""; controls();
    try { await action(); } catch (error) { $("error").textContent = error.message; }
    finally { busy = false; controls(); schedule(); }
  }
  function schedule() {
    stopPolling();
    if (dialog.open && jobId && ongoing.has(current?.status)) {
      timer = setTimeout(() => run(load), 2500);
    }
  }
  ...
  function render(data) {
    ...
    $("summary").textContent = `发现 ${summary.discovered || 0} · 已选择 ${summary.selected || 0} · 已入库 ${summary.imported || 0} · 处理中 ${summary.active || 0} · 已完成 ${summary.completed || 0} · 失败 ${summary.failed || 0}`;
    ...
    // Preserve scope selections between refreshes of the same catalog.
    ...
    if (!data.folders?.length) $("folders").append(node("p", "暂无可识别的收藏夹。可选择全部收藏，或先读取目录。"));
    ...
    for (const item of data.items || []) {
      ...
      } catch (_) { /* Show malformed URLs as plain text. */ }
      content.append(title, node("small", `${item.author || "未知作者"} · ${item.source_kind_label || item.source_kind} · ${item.disposition_label || item.disposition}${item.job_status_label ? ` · ${item.job_status_label}` : ""}`));
      if (item.error_message) content.append(node("small", item.error_message));
      row.append(input, content); $("items").append(row);
    }
    if (!data.items?.length) $("items").append(node("p", data.directory_only ? "目录读取完成后，可选择收藏夹并生成作品清单。" : "当前筛选下暂无作品。"));
  ...
  async function load() {
    ...
    if (token === generation && dialog.open) render(data);
  }
  async function start(directoryOnly) {
    ...
    const result = await request(base, {directory_only: directoryOnly, include_images: $("images").checked,
    ...
    await load(); await history(); dialog.scrollTop = 0;
  }
  $("toggle").addEventListener("click", () => { dialog.showModal(); run(async () => { await history(); if (jobId) await load(); }); });
  $("close").addEventListener("click", () => dialog.close());
  dialog.addEventListener("close", () => { stopPolling(); generation++; });
  ...
  $("confirm").addEventListener("click", () => run(async () => {
    await request(`${base}/${encodeURIComponent(jobId)}/confirm`, {accept_partial: $("partial").checked}); await load(); await history();
  }));
